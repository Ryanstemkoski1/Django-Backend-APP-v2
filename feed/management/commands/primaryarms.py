...
from feed.models import PrimaryArms
...
class Command(BaseCommand):
    help = 'Build PrimaryArms Database'
    ...
    def getData(self):
        ...
        f = open(FILEDIR + "/files/PrimaryArms.csv", "rb")
        ...
        for row in cr:
            ...
            try:
                product = PrimaryArms.objects.create(
                    sku=row[0],
                )
            ...
            product.medium_image_url = row[20]

            product.content_widget = row[21]

            product.google_categorization = row[22]
            try:
                product.commission = float(row[23])
            except:
                pass
        ...
        debug("PrimaryArms", 0, "Completed Processing PrimaryArms Data Feed")
