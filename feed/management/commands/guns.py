...
class Command(BaseCommand):
    ...
    def getData(self):
        ...
        f = open(FILEDIR + "/files/Guns.csv", "rb")
        cr = csv.reader(codecs.iterdecode(f, encoding="ISO-8859-1"))

        for row in cr:
            if row[0] == 'SKU' or row[0] == 'KGSQXGE6PB' or row[3] == '':
                continue

            try:
                product = Guns.objects.create(
                    sku=row[0],
                )
        ...
        debug("Guns", 0, "Completed Processing Guns Data Feed")
